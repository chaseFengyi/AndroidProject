...
import com.CollegeState.BuyActivity.MenuActivity;
...
/**
 * 处理推送信息，转发推送规则 This class is to notify the user of messages with
 * NotificationManager.
 * 
...
public class Notifier {
	...
	public void notify(String notificationId, String apiKey, String title,
		...
		XMPPBean xb = JSONUtils.toXmppMessage(title, message);
		//解析JSON失败
		if(xb == null){
			return;
		}
		if (isNotificationEnabled()) {
			...
			notification.tickerText = xb.getMessage();
			...
			intent.putExtra("shopId", xb.getShopId());
			intent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
			...
			notification.setLatestEventInfo(context, xb.getShopName(),
					xb.getMessage(), contentIntent);
